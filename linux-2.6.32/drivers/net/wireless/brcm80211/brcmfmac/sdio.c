...
/* in RHEL, we could use atomic_set_mask(), but PowerPC does not have
 * this primitive.
 */
#if 1
#ifdef CONFIG_SMP 
static inline void x_atomic_or(int i, atomic_t *v)
{
	int c, old;
...
#else
		x_atomic_or(intstatus, &bus->intstatus);
#endif
...
#else
	wq = create_singlethread_workqueue(dev_name(&sdiodev->func[1]->dev));
#endif
	if (!wq) {
